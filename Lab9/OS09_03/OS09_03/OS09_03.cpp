...
int main()
{
    ...
    LPWSTR fileName = (LPWSTR)L"D:\\Education\\3 курс 2 сем\\OS\\Lab9\\OS09_01.txt";

    printFileTxt(fileName);

    insRowFileTxt(fileName, (LPWSTR)L"1.Апаланюк Антон Вадимович(0)", 0);
    insRowFileTxt(fileName, (LPWSTR)L"4.Буданова Ксения Андреевна(-1)", -1);
    insRowFileTxt(fileName, (LPWSTR)L"TESTSTRING(4)", 4);
    insRowFileTxt(fileName, (LPWSTR)L"10.Димитриади Антон Владимирович(7)", 7);

    printFileTxt(fileName);

}

BOOL insRowFileTxt(LPWSTR FileName, LPWSTR str, DWORD row)  //вставить строку в текстовом файле FileName
{
...
BOOL printFileTxt(LPWSTR FileName)  //выводит в стандартный поток вывода содержимое файла.
{
